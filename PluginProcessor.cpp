...
//==============================================================================
void VariableSizedBuffer::clone(const dsp::AudioBlock<float>& other)
{
    clear(other);
...
window.multiplyWithWindowingTable (fftData, FFTSizes::fftSize);      // [1]
    ...
    for (int i = 0; i < numPoints; ++i)                        // [3]
    {
        ...
        auto fftDataIndex = jlimit (0, FFTSizes::fftSize / 2, (int) (skewedProportionX * FFTSizes::fftSize / 2));
        auto level = jmap (jlimit (mindB, maxdB, Decibels::gainToDecibels (fftData[fftDataIndex])
                                               - Decibels::gainToDecibels ((float) FFTSizes::fftSize)),
...
void BufferAnalyzer2::paint(Graphics& g)
{
    ...
    for(int i = 1; i < FFTSizes::numPoints; ++ i)
    {
        ...
        auto endX = jmap((float)i, 0.f, float(FFTSizes::numPoints), 0.f, w);
